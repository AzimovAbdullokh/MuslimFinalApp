...
import com.example.audioservice.service_player.presentation.PlayerViewModel
import com.example.audioservice.service_player.presentation.models.AudioNasheedsUi
import com.example.audioservice.service_player.service_player.*
...
//        }
    ...
    private fun observeData() = with(playerViewModel) {
        ...
        audioNasheedFlow
            .onEach {
                if (it == null)
                if (it == AudioNasheedsUi.unknown()) dismissPlayerOverlay()
            }
            .onEach(viewModel::startRenderContent)
            .launchIn(lifecycleScope)

        audioNasheedIdFlow.launchIn(lifecycleScope)
...
//        (window.callback as? PlayerWindowCallback)
//            ?.originalCallback
